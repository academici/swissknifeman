...
Полный паттерн (атрибут, резолвер, трейт, доменные оси) — скилл `php/enum-attributes`.
...
Шаблон подключения — `php/enum-attributes` → `snippets/composer-path-repo.json`; устройство пакета — скиллы `php/laravel-package-*`.
