...
- **Межпроектный агент-оптимизатор** — поверх карты топологии
  (`~/.swissknifeman/topology.json`, см. скилл `system/local-topology`) агент
  обходит все связанные проекты в базе проектов, ищет общие части
  (дублирующийся код, расходящиеся реализации одного и того же, кандидаты в
  общий пакет) и предлагает унификацию в единую кодовую базу. Топология даёт
  узлы и точки входа; Brain — связь по документации. Цель — межпроектное
  взаимодействие, направленное на общее улучшение, а не оптимизация в рамках
  одного репозитория.
